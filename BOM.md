...
| Alimentation Pi automobile | candidat encapsulé annoncé `8–32 V DC → 5 V / 3 A`, USB-C captif ~28 cm, rendement annoncé jusqu'à 92 %, protections surcharge/surchauffe/surintensité, IP68; à valider sous charge réelle sur Pi 4 | 1 | SELECT | P1 | ~4 € vu AliExpress 2026-09-08 |
...
- pour le Pi 4, **ne jamais injecter le 12 V véhicule directement** : utiliser un buck automobile stabilisé vers `5 V`; alimentation par USB-C préférée pour le prototype/final, plutôt que par les pins 5 V GPIO ;
- le candidat `8–32 V → 5 V / 3 A` est intéressant pour le démarrage moteur grâce à son seuil d'entrée annoncé à 8 V, mais ses protections contre les transitoires automobiles >32 V ne sont pas documentées : conserver fusible + TVS/protection amont dans l'architecture finale ;
